...
public class BloomJoin {
	...
	private static HashMap<String, R3Elements> map;
	...
	public static void main(String args[]) throws IOException {
		...
		map = new HashMap<String, R3Elements>();
		System.out.println("Start to read r1 into Bloom Filter");
		String relation1path = "A:\\COMS535\\Relation1.txt";
		String relation2path = "A:\\COMS535\\Relation2.txt";
		String relation3path = "A:\\COMS535\\Relation3.txt";
		String finalJoinPath = "A:\\COMS535\\FinalJoin.txt";
	 ...
	 * @param relation3path
	 * */
	private static void addR3IntoHashMap(HashMap<String, R3Elements> map, String relation3path){
		try {
			int prev = 0;
			int hasPut = 0;
			...
			while(line != null){
				...
				//because there may have several same attribute to different values
				//we need to add them into HashMap as encapsulated Class R3Elements
				//in order to store all the value of a same attributes
				if(map.containsKey(attribute)){
					map.get(attribute).values.add(value);
				}else{
					R3Elements element = new R3Elements(attribute);
	 ...
	 * 
	 * */
	private static void joingR1AndR3(HashMap<String, R3Elements> map, String relation1path,  String finalJoinFilePath){
		try {
			int prev = 0;
			int hasJoin = 0;
			...
			while(line != null){
        		...
        		if(map.containsKey(attribute)){
        			ArrayList<String> values = map.get(attribute).values;
        			/*
        			 * find all the values of this attribute, then join them 
        			 * with R1.
        			 * */
        			for(int i = 0; i < values.size(); i++){
                		...
                		bufferedWriter.write("   ");
                		bufferedWriter.write(map.get(attribute).values.get(i));
                		bufferedWriter.newLine();
        				hasJoin++;
...
/**
 * 
 * in order to prevent the situation when, in R3, several lines have same attributes,
 * but have different values.
 * 
 * If we directly store the value into hashMap, the previous value will be covered
 * by the upcoming values. That will result in a lost of information.
 * 
 * So we store all the value of a same attributes into this encapsulated class
 * to protect values with same attributes.
 * 
 * */
class R3Elements{
	ArrayList<String> values;
	String key;
	R3Elements(String key){
		this.key = key;
